...
test.describe('Animation Feature', () => {
    test('should start and stop animation', async ({ page }) => {
        ...
        const box = page.locator('.feature-container:has-text("Feature D: Animation") > div').nth(0);
        ...
        const movingTransform = await box.evaluate((el) => window.getComputedStyle(el).transform);
        ...
        const stoppedTransform = await box.evaluate((el) => window.getComputedStyle(el).transform);

        // Wait more and check if it still moves
        await page.waitForTimeout(500);
        const finalTransform = await box.evaluate((el) => window.getComputedStyle(el).transform);
    ...
    test('should handle search with stale responses', async ({ page }) => {
        ...
        await expect(page.locator('text=Status: success')).toBeVisible({ timeout: 10000 });
    ...
    test('should compute primes in worker', async ({ page }) => {
        ...
        await expect(page.locator('text=Status: done')).toBeVisible({ timeout: 10000 });
    ...
    test('should work with DevTools replay', async ({ page }) => {
        ...
        const countText = await page.locator('.feature-container:has-text("Feature C: Timer") >> p').innerText();

        await replayBtn.click();
        await expect(page.locator('.replay-result')).toContainText('success', { timeout: 10000 });

        const finalCountText = await page.locator('.feature-container:has-text("Feature C: Timer") >> p').innerText();
