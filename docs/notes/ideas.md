...
- **Synchronous Re-entrancy**: Dispatched messages triggered by synchronous effects are strictly FIFO. If `Update(A)` triggers `Effect(B)`, `Dispatch(B)` is queued and processed *after* the current loop tick (or next in queue), ensuring strictly predictable state transitions (`A -> B -> C`).
