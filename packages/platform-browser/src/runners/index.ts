import { Msg, CoreEffect, FetchEffect, TimerEffect, AnimationFrameEffect, WorkerEffect } from '@causaloop/core';
...
export class BrowserRunner {
    ...
    private runFetch(effect: FetchEffect, dispatch: (msg: Msg) => void): void {
        const { url, method = 'GET', headers, body, expect = 'json', timeoutMs } = effect;
        ...
        fetch(url, fetchOptions)
            ...
            .finally(() => {
                if (effect.abortKey && this.controllers.get(effect.abortKey) === controller) {
    ...
    private runAnimationFrame(effect: AnimationFrameEffect, dispatch: (msg: Msg) => void): void {
